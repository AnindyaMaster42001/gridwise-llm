...
Required behaviour
------------------
1. Ask the interpreter for one raw directive per note (Ninad).
2. Run guardrails over the raw output (Kabya). Invalid entries are repaired
   where the repair is unambiguous, otherwise demoted to no_op — never dropped,
   never invented.
3. Build the ConstraintSet and solve (Kabya).
4. Verify the plan against the ConstraintSet before returning (Kabya). If the
...
async def run_pipeline(payload: ScenarioRequest, request_id: str = "") -> OptimizeResponse:
    ...
    raise NotImplementedError("TODO(Anindya): see docs/team/ANINDYA-lane-a-api-orchestration.md")
...
def summarise_totals(plan: List[HourPlan], tariffs: List[float]) -> tuple[float, float, float]:
    ...
    nowhere else.
    """
    raise NotImplementedError("TODO(Anindya)")
